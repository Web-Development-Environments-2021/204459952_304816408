...
const InitialzeAllGhosts = () => {
  ...
  for (let i = 0; i < ghotsAmount; i++) {
    ...
    if (i == 0) {
      ...
      g_board[1][size_of_board - 2] = '18';
    } else if (i == 2) {
      ghosts[i].ghost_number = '10';
      ...
      g_board[size_of_board - 2][1] = '10';
    } else if (i == 3) {
      ghosts[i].ghost_number = '17';
      ...
      g_board[size_of_board - 2][size_of_board - 2] = '17';
    }
...
const takeStepAtRandom = (i) => {
  ...
  while (true) {
    ...
    if (
      ...
      board[ghosts[i].row + 1][ghosts[i].col] != '4'
    ) {
      return keyNum.Down;
    } else if (
      rand_generated_number >= 0.25 &&
      ...
      board[ghosts[i].row][ghosts[i].col + 1] != '4'
    ) {
      return keyNum.Right;
    } else if (
      rand_generated_number >= 0.75 &&
...
const Draw = (packmanSide) => {
  ...
  for (var i = 0; i < size_of_board; i++) {
    for (var j = 0; j < size_of_board; j++) {
      ...
      } else if (board[i][j] == '12' && s.appearnce) {
        context.drawImage(
          s.image,
          center.x - cell_width / 2,
          center.y - cell_height / 2,
...
const UpdatePosition = () => {
  ...
  if (board[shape.i][shape.j] == '16' && d.appear) {
    d.appear = false;
    score += 50;
  }
  if (board[shape.i][shape.j] == '5' && s.appearnce) {
    NumberOfdLives++;
    s.times--;
  }
...
const gPositionUpdater = () => {
  ...
  for (let i = 0; i < ghotsAmount; i++) {
    ...
    while (true) {
      rand_generated_number = Object.keys(keyNum).length * Math.random();
      let generated_m = Math.floor(rand_generated_number);

      if (ghostMoveValidator(i, generated_m)) {
...
const gStepUpdate = (i, step) => {
  g_board[ghosts[i].row][ghosts[i].col] = '0';

  ghosts[i].row += s_Matrix[0][step];
  ghosts[i].col += s_Matrix[1][step];

  if (step == keyNum.Right) {
    ghosts[i].draw = ghosts[i].image_right;
  } else if (step == keyNum.Left) {
...
const ramdomLocatin = () => {
  return Math.floor(Math.random() * (size_of_board / 2) + 1);
};
...
const setSpecialFood = () => {
  ...
  d.previos_step = keyNum.Down;
  d.appear = true;
  //choose rand number step
  rand_generated_number = Math.random();
  ...
  board[loaction.row][loaction.col] = '16';
};
